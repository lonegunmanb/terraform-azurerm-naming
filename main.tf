...
locals {
  ...
  az = {
    ...
    linux_function_app = {
      name        = substr(join("-", compact([local.prefix, "linfunc", local.suffix])), 0, 60)
      name_unique = substr(join("-", compact([local.prefix, "linfunc", local.suffix_unique])), 0, 60)
      dashes      = true
      slug        = "linfunc"
      min_length  = 2
      max_length  = 60
    ...
    windows_function_app = {
      name        = substr(join("-", compact([local.prefix, "winfunc", local.suffix])), 0, 60)
      name_unique = substr(join("-", compact([local.prefix, "winfunc", local.suffix_unique])), 0, 60)
      dashes      = true
      slug        = "winfunc"
      min_length  = 2
      max_length  = 60
