...
fmtcheck: tfvalidatecheck tffmtcheck gofmtcheck terrafmtcheck jsonfmtcheck
...
jsonfmt:
	@sh "$(CURDIR)/scripts/jsonfmt.sh"

jsonfmtcheck:
	@sh "$(CURDIR)/scripts/jsonfmt-check.sh"

goimports:
	@echo "==> Fixing imports code with goimports..."
	@find . -name '*.go' | grep -v vendor | while read f; do ./scripts/goimport-file.sh "$$f"; done

pre-commit: tffmt terrafmt goimports fmt fumpt generate jsonfmt
...
gencheck:
	...
	@sh "$(CURDIR)/scripts/go-generate-check.sh"
